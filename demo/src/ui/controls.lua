...
function init_text_input( c_win, is_pwd, onchange )
    ...
    function obj:setInputMode( input_model ) self.c_win:setInputMode( input_model or TL_INPUT_TEXT_MODEL_NORMAL ) end
...
function init_progress_bar( node, min, max )
    ...
    function pb:addSubBar( tex, init_value, is_flip )
        ...
        function sub_bar:setPercent( percent )
            if is_flip then
                clip_region.size.width = child_frame.mcBoundingBox.size.width * percent
                clip_region.origin.x = child_frame.mcBoundingBox.origin.x + ( child_frame.mcBoundingBox.size.width - clip_region.size.width )
            else
                clip_region.size.width = child_frame.mcBoundingBox.size.width * percent
            end
...
function progressBar_FromMCFrame( frame, tex, min, max )
    ...
    local clipRegion = copy_rect( frame.mcBoundingBox )
    return {
        ...
        setValue = function( value )
            ...
            clipRegion.size.width = width
            frame:setClipRegion( clipRegion )
        end,
        setPercent = function(p)
            -- 0 <= p <= 1
            clipRegion.size.width = frame.mcBoundingBox.size.width * p
            frame:setClipRegion( clipRegion )
        end,
    }
end
