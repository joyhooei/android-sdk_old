...
function __check_upgrade:ctor( check_info )
    self.file_list_url = string.format( '%s%d_%s.list', GameSettings.PACKAGE_URL, game_version, getIncrementalFileSuffix() )
...
function __check_upgrade:upgradeCompleteCallback( complete_type, check_upgrade_response )
    ...
    if complete_type == 'update_complete' then
        ...
        for _,file_info in ipairs( check_upgrade_response.files ) do
            table.insert( file_data.files, { url = file_info.url, md5 = file_info.md5, size = file_info.size, where = 'INTERNAL', } )
            delta[file_info.url] = 1
        end

        local old_file_list = load_file_list()
        for _, file_info in ipairs( old_file_list.files ) do if not delta[file_info.url] then table.insert( file_data.files, file_info ) end end
...
function __check_upgrade:startDownload( check_upgrade_response )
    ...
    for i,file_info in ipairs( check_upgrade_response.files ) do
        if file_info.size > 0 then
            ...
            if not CCFileUtils:sharedFileUtils():checkFileExists( external_path .. path, file_info.size ) then
    ...
    self:prepareDownloadCallback( function()
        local cur_index = 10
        local __request_file = function( index )
            local file_info = download_list[index]
            if not file_info then return end

            lfs.mkdir( file_info.dir )

            local download_url = GameSettings.PACKAGE_URL .. file_info.path
            TLHttpClient:sharedHttpClient():requestFile( download_url, '', function( content_data, http_code, error_code, error_msg )
                local save_to = external_path .. file_info.path
                if http_code == 200 then
                    -- 写文件
                    local file, err_msg = io.open( save_to, 'wb' )
                    if not file then
                        local err_report = string.format( 'open %s failed ! err_msg : %s ', save_to, tostring( err_msg ) )
                        require 'utils.netmessage'
                        sendHTTPNetMsgEx( GameSettings.ERROR_REPORT_URL, '', err_report, function() end )
                        CCLuaLog( err_report )
                    else
                ...
                -- 写入的大小不正确
                if not CCFileUtils:sharedFileUtils():checkFileExists( save_to, file_info.size ) then failed_count = failed_count + 1 end
