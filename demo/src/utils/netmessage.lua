...
local function encode_msg( msg_id, msg_tbl )
    ...
    if not msg_tbl then return nil, nil, lock_info, nm_info.recv_func end
    if not nm_info.encode_proto then return nil, nil, lock_info, nm_info.recv_func end

    local msg = protobuf.encode( nm_info.encode_proto, msg_tbl )
    if msg then
        return msg, #msg, lock_info, nm_info.recv_func
    else
        return nil, nil, lock_info, nm_info.recv_func
    end
end
...
local function sendNetMsgPreviousFunc( lock_info )
    ...
    if lock_info and lock_scene_func then lock_scene_func( lock_info ) end
...
local function recvNetMsgAfterFunc( lock_info )
    ...
    if lock_info and unlock_scene_func then unlock_scene_func( lock_info ) end
...
function sendHTTPNetMsg( msg_id, msg_tbl, call_back_func, err_call_back_func )
    ...
    TLHttpClient:sharedHttpClient():sendMsg( msg_id, function( msg_content, http_code, error_code, error_msg )
        if http_code ~= 200 then
            ...
            handleHttpErrorCode( msg_id, http_code, error_code, error_msg )
...
function sendHTTPNetMsgEx( url, content_type, msg_data, call_back_func )
    TLHttpClient:sharedHttpClient():sendMsg( -1, function( msg_content, http_code, error_code, error_msg )
        CCLuaLog( 'sendHTTPNetMsgEx url : ' .. tostring( url ) )
        CCLuaLog( 'sendHTTPNetMsgEx http_code : ' .. tostring( http_code ) )
        CCLuaLog( 'sendHTTPNetMsgEx error_code : ' .. tostring( error_code ) )
        CCLuaLog( 'sendHTTPNetMsgEx error_msg : ' .. tostring( error_msg ) )
...
function registerNetMsg( msg_id, encode_proto, decode_proto, recv_func, lock_info )
    net_msg_info[msg_id] = {
        encode_proto = encode_proto,
        decode_proto = decode_proto,
        lock_info = lock_info
...
function sendNetMsg( msg_id, msg_tbl, call_back_func )
    local msg,len,lock_info = encode_msg( msg_id, msg_tbl )
...
function initHandleDefaultError( msg )
    ...
    CCLuaLog( 'msg_id = ' .. tostring( msg_id ) )
    CCLuaLog( 'real_msg_id = ' .. tostring( real_msg_id ) )
    CCLuaLog( 'err_code = ' .. tostring( err_code ) )
    CCLuaLog( 'err_str = ' .. tostring( err_str ) )
    if g_welcome_loading_obj and g_welcome_loading_obj.win and toTLWindow(g_welcome_loading_obj.win) then
        g_welcome_loading_obj:close()
    end

    if g_request_fight_loading_obj and g_request_fight_loading_obj.win and toTLWindow(g_request_fight_loading_obj.win) then
        g_request_fight_loading_obj:close()
    end
end
...
function handleSocketDisconnected( err_code, msg_id, reason )
    CCLuaLog( 'err_code : ' .. tostring( err_code ) )
    CCLuaLog( 'msg_id : ' .. tostring( msg_id ) )
    CCLuaLog( 'reason : ' .. tostring( reason ) )
    if time_out_handle then
        unCommonSchedule( time_out_handle )
        time_out_handle = nil
    end
...
function handleHttpErrorCode( msg_id, http_code, error_code, error_msg )
    ...
    CCLuaLog( 'handleHttpErrorCode msg_id : ' .. tostring( msg_id ) )
    CCLuaLog( 'handleHttpErrorCode http_code : ' .. tostring( http_code ) )
    CCLuaLog( 'handleHttpErrorCode error_code : ' .. tostring( error_code ) )
    CCLuaLog( 'handleHttpErrorCode error_msg : ' .. tostring( error_msg ) )

    local err_code = tonumber( error_code )
    CCLuaLog( 'err_code : ' .. tostring( err_code ) )
    if err_code then openErrorByErrorCode( err_code, msg_id, error_msg ) end
    --handleSocketDisconnected( err_code, msg_id, error_msg )
end
...
function openErrorByErrorCode( errorCode, msgId, reason )
    ...
    if errInfo ~= nil then
        ...
        local err_str = string.format( 'Undeclared error code : [%s(%s)]', tostring( errorCode ), tostring(msgId) )
        --openMessageBox( '', err_str, 'MB_OK', {
        --    MB_LAYER = function()
        --        return layer_type_mask
        --    end,
        --})
        showFloatTip(err_str)
    end
end
