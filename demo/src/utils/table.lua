...
function LuaSequence()
    local ret = {datas = {},len=0,head=1}
    function ret:append(obj)
        self.datas[self.head + self.len] = obj
        self.len  = self.len  + 1
    end

    function ret:pop()
        if self.len <= 0 then return end
        local obj = self.datas[self.head]
        self.datas[self.head] = nil
        self.len  = self.len  - 1
        if self.len <= 0 then
            self.head = 1
        else
