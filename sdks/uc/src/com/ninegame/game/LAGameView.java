...
public class LAGameView extends SurfaceView implements SurfaceHolder.Callback {
	...
	public LAGameView(Activity activity, boolean isLandScape) {
		...
		this.screenImage = new LAImage(width = handler.getWidth(),
	...
	class CanvasThread extends Thread {
		public void run() {
			...
			do {
				...
				// 设置上次更新所花时间
				timerContext.setSinceLastUpdateTime(curTime
						- timerContext.getMillisTime());

				// 设置睡眠时间
				timerContext.setMillisSleepTime((offsetTime - timerContext
						.getSinceLastUpdateTime())
						- timerContext.getMillisOverSleepTime());
				if (timerContext.getMillisSleepTime() > 0)// ���ÿ֡�ļ��̫�̣�������ʱ��
				{
					...
					timerContext.setMillisOverSleepTime((System
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int width,
		...
		GLog.d(TAG, "surfaceDestroyed (width:%d, height:%d)", this.getWidth(),
				this.getHeight());
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		GLog.d(TAG, "surfaceCreated (width:%d, height:%d)", this.getWidth(),
				this.getHeight());
		isSurfaceActive = true;
		ILAScreen screen = handler.getScreen();
		if (screen != null) {
			screen.beforeDraw(this);
		}
		resumeCanvasThread();
	}
