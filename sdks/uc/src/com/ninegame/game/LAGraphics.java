...
public class LAGraphics {
	...
	public void drawImage(LAImage img, int x, int y, int w, int h, int x1,
			int y1, int w1, int h1) {
		canvas.drawBitmap(img.getBitmap(), new Rect(x1, y1, w1, h1), new Rect(
				x, y, w, h), paint);
	}
	...
	public void copyArea(int srcX, int srcY, int width, int height, int dx,
			int dy) {
		Bitmap copy = Bitmap.createBitmap(bitmap, srcX, srcY, width, height);
		canvas.drawBitmap(copy, srcX + dx, srcY + dy, null);
	}

	public void switchPlace(int srcX, int srcY, int dstX, int dstY, int width,
			int height) {
