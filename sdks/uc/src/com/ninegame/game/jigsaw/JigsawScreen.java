...
import com.ninegame.game.LAGraphics;
import com.ninegame.game.LAImage;
import com.ninegame.game.LAScreen;
...
public class JigsawScreen extends LAScreen {
	...
	public JigsawScreen(LAGameView gameView, String mainImage,
			String finishImage, int row, int col) {
		this.colCount = col;
		this.rowCount = row;
	...
	private void copy(int srcX, int srcY, int x2, int y2) {
		tmp_graphics.copyArea(srcX * cellWidth, srcY * cellHeight, cellWidth,
				cellHeight, (x2 - srcX) * cellWidth, (y2 - srcY) * cellHeight);
	}
		...
		if (!isEvent)// �״μ���
		{
			...
			for (i = 0; i < rowCount; i++) {
				for (j = 0; j < colCount; j++) {
					g.drawRect(i * cellWidth, j * cellHeight, cellWidth,
							cellHeight);
				}
			}
		}
	...
	public boolean onTouchDown(MotionEvent e) {
		...
		if (lastClickCell != null) {
			if ((srcX == lastClickCell.x) && (srcY == lastClickCell.y)) {
				tmp_graphics.cancelHighLight(lastClickCell.x * cellWidth,
						lastClickCell.y * cellHeight, cellWidth, cellHeight);
			} else {
				tmp_graphics.cancelHighLight(lastClickCell.x * cellWidth,
						lastClickCell.y * cellHeight, cellWidth, cellHeight);
				tmp_graphics.switchPlace(srcX * cellWidth, srcY * cellHeight,
						lastClickCell.x * cellWidth, lastClickCell.y
								* cellHeight, cellWidth, cellHeight);
				int swapVar = blocks[srcY * rowCount + srcX];
				blocks[srcY * rowCount + srcX] = blocks[lastClickCell.y
						* rowCount + lastClickCell.x];
			...
			tmp_graphics.highLight(srcX * cellWidth, srcY * cellHeight,
	...
	@Override
	public void beforeDraw(SurfaceView view) {
		this.imageBack = LAGraphicsUtils.resizeImage(
				getLAImage(this.strMainImageFile), view.getWidth(),
				view.getHeight());
