...
public class LAGraphicsUtils {
	private static final Map<String, LAImage> cacheImages = new HashMap<String, LAImage>(
			100);
	...
	public static LAImage resizeImage(LAImage image, int w, int h) {
		...
		Bitmap resizedBitmap = Bitmap.createBitmap(image.getBitmap(), 0, 0,
				width, height, matrix, true);
		return new LAImage(resizedBitmap);
	}

	public static LAImage[] getSplitImages(String fileName, int width,
			int height) {
		return getSplitImages(LAGraphicsUtils.loadLAImage(fileName), width,
				height);
	}
